...
namespace DatabaseReference
{
    public class MyDatabase
    {
        ...
        public void InsertIntoDB(int id,string name,string type)
        {
            ...
            if (type == "savings")
            {
                sqlInsertQuery = "insert into Account (Account_Number,Full_Name,Amount,Account_Type) values(" + id + ",'" + name + "'," + 1000 + ",'" + type + "')";
                SqlCommand sqlCommand = new SqlCommand(sqlInsertQuery, connection);
                sqlCommand.ExecuteNonQuery();
            }
            else if (type == "current")
            {
                sqlInsertQuery = "insert into Account (Account_Number,Full_Name,Amount,Account_Type) values(" + id + ",'" + name + "'," + 0 + ",'" + type + "')";
                SqlCommand sqlCommand = new SqlCommand(sqlInsertQuery, connection);
                sqlCommand.ExecuteNonQuery();
            }
            connection.Close();
        ...
        {
            SqlConnection connection = CreatingConnection();
            string sqlSelectQuery = "select * from Account";
            SqlCommand sqlCommand = new SqlCommand(sqlSelectQuery, connection);
            SqlDataReader sqlReader = sqlCommand.ExecuteReader();
            
            while (sqlReader.Read())
            {
                Console.Write("Account Number :- {0} \nFull Name :- {1} \nAmount :- {2} \nAccount Type  :- {3}", sqlReader.GetValue(0), sqlReader.GetValue(1), sqlReader.GetValue(2), sqlReader.GetValue(3));
                Console.WriteLine();
                Console.WriteLine();
            }
            connection.Close();
        ...
        public string SelectRow(int id)
        {
            ...
            SqlConnection connection = CreatingConnection();
            string sqlSelectQuery = "select * from Account where Account_Number=" + id;
            SqlCommand sqlCommand = new SqlCommand(sqlSelectQuery, connection);
            SqlDataReader sqlReader = sqlCommand.ExecuteReader();
            if (sqlReader.Read())
            {
                output += sqlReader.GetValue(0);
                output += "\n Full Name is :- ";
        ...
        {
            SqlConnection connection = CreatingConnection();
            string sqlUpdateQuery = "update Account set Amount=Amount+" + money + "where Account_Number="+id;
        ...
        public void WithdrawAmount(int id,int money)
        {
            string updateQuery = "";
            SqlConnection connection = CreatingConnection();
            string checkBalanceQuery = "select Amount,Account_Type from Account where Account_Number=" + id;
            ...
            if (flag == 1)
            {
                ...
                updateQuery = "update Account set Amount=Amount-" + money + " where Account_Number=" + id;
